...
<html:html>
    ...
    <body>
    <input type="hidden" name="ixsact" id="ixsact" value="
    <%
       if(session.getAttribute("xid")!=null && Integer.parseInt(session.getAttribute("xid").toString())>0)
        out.print(session.getAttribute("xid"));
    %>
    "/>
        <tiles:insert attribute="BodyTop"></tiles:insert>
        <tiles:insert attribute="BodyCenter"></tiles:insert>
        <%if( session.getAttribute("xid")== null )
           {%>
